...
/* ─── Auto Backlink Creator (REAL PINGING) ─── */
...
const autoBacklink = async (req, res) => {
  ...
  try {
    // Extract pure domain from URL (e.g. https://www.example.com/post -> example.com)
    let domain = url.replace(/^(?:https?:\/\/)?(?:www\.)?/i, "").split('/')[0];
    
    // Real indexing/stat websites that create automatic profile pages for queried domains
    const statSites = [
    ...
    const selectedSites = statSites.sort(() => 0.5 - Math.random()).slice(0, num);

    const backlinks = await Promise.all(selectedSites.map(async (site) => {
      const liveUrl = site.template.replace('{domain}', domain);
      
      // Ping the URL to ensure it gets generated by their system (fire and forget)
      try {
